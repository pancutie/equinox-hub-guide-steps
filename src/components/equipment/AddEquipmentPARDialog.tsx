...
interface AddEquipmentPARDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (item: Omit<EquipmentPAR, 'id'>) => void;
}

export default function AddEquipmentPARDialog({
  isOpen,
  onClose,
  onSave
}: AddEquipmentPARDialogProps) {
  const [formData, setFormData] = useState<Omit<EquipmentPAR, 'id'>>({
    propertyNo: "",
    description: "",
    quantity: 1,
    unit: "",
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle className="text-xl text-purple-800">Add New Equipment (PAR)</DialogTitle>
          <DialogDescription>
