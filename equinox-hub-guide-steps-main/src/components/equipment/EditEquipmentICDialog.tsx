...
interface EditEquipmentICDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (item: EquipmentIC) => void;
  equipment: EquipmentIC;
}

export default function EditEquipmentICDialog({
  isOpen,
  onClose,
  onSave,
  equipment
}: EditEquipmentICDialogProps) {
  const [formData, setFormData] = useState<EquipmentIC>({ ...equipment });
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle className="text-xl text-purple-800">Edit Equipment (ICS)</DialogTitle>
          <DialogDescription>
